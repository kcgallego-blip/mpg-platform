...
export default function SupportRowForm({ category, row, busy, onCancel, onSubmit }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/35 p-4 backdrop-blur-sm">
      <form onSubmit={submit} className="max-h-[90vh] w-full max-w-2xl overflow-auto rounded-2xl border border-outline-variant bg-surface p-6 shadow-2xl">
          ...
          return <div key={column.key}>
            <div className="mb-1.5 flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-1 rounded-lg border border-outline-variant bg-surface-container-low/50 p-1" aria-label={`${column.label} text formatting`}>
                <button type="button" onClick={() => updateFormat(column.key, { bold: !format.bold })} aria-pressed={Boolean(format.bold)} title="Bold" className={`rounded p-1.5 ${format.bold ? 'bg-primary-container text-on-primary-container' : 'text-on-surface-variant hover:bg-surface-container-low'}`}><Bold size={15} /></button>
                <button type="button" onClick={() => updateFormat(column.key, { italic: !format.italic })} aria-pressed={Boolean(format.italic)} title="Italic" className={`rounded p-1.5 ${format.italic ? 'bg-primary-container text-on-primary-container' : 'text-on-surface-variant hover:bg-surface-container-low'}`}><Italic size={15} /></button>
                <label className="relative h-7 w-8 cursor-pointer overflow-hidden rounded border border-outline-variant bg-surface" title="Text color"><span className="sr-only">{column.label} text color</span><input type="color" value={format.color || '#475569'} onChange={event => updateFormat(column.key, { color: event.target.value })} className="absolute -inset-2 h-11 w-12 cursor-pointer border-0 p-0" /></label>
                <button type="button" onClick={() => updateFormat(column.key, { color: undefined, bold: false, italic: false })} title="Clear formatting" className="rounded p-1.5 text-on-surface-variant hover:bg-surface-container-low"><RotateCcw size={14} /></button>
              </div>
            </div>
